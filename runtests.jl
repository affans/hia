...
@testset "Age/Death" begin
  ...
  x = humans[1]
  println("age: $((x.age)/365)")
  println("adeath: $((x.agedeath)/365)")
  println("")
  # for i = 1:length(humans)
  #   @test humans[i].age <= humans[i].agedeath
  # end
  ...
  # x =humans[1]
  # x.age = 90*365
  # x.agedeath = rand(distribution_death(x.age))
  # println("age: $(x.age)")
  # println("adeath: $(x.agedeath)")

  ## get median age
  a = zeros(Int64, 100000)
  for i = 1:100000
    a[i] = humans[i].age
  end
  println("median: $(median(a)/365)")

  ## check if humans are dying
  humans = setuphumans(1, P, M)
  x = humans[1]
  x.age = x.agedeath
  app(x, P)
  @test (x.age + 1) == x.agedeath
  @test x.swap == DEAD
end

## to do, see if costs for invasive are adding up correctly. 